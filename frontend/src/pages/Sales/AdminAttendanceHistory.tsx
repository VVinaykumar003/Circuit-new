...
export default function AdminAttendanceHistory() { 
  ...
  function handleBulkApprove() {
    const recordsToApprove = records.filter(r => selectedIds.has(r._id));
    recordsToApprove.forEach((record) => {
      // Use the parent document ID for the mutation
      approveMutation.mutate({ attendanceId: record.attendanceDocId, employeeId: record.employee._id });
    });
    setSelectedIds(new Set());
  }

  function handleBulkReject() {
    const recordsToReject = records.filter(r => selectedIds.has(r._id));
    recordsToReject.forEach((record) => {
      // Use the parent document ID for the mutation
      rejectMutation.mutate({ attendanceId: record.attendanceDocId, employeeId: record.employee._id });
    });
    setSelectedIds(new Set());
  }
