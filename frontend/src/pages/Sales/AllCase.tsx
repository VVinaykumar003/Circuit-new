...
export default function AllCase() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "timeline" | "comments" | "attachments" | "resolution"
  >("overview");
  ...
  const [commentType, setCommentType] = useState<"Internal" | "Customer">(
    "Internal",
  );
  ...
  const salesReps = useMemo(
    () =>
      repsData?.data
        ?.map((r: any) => r.memberId?.name || r.name || r.fullName)
        .filter(Boolean) || [],
    [repsData],
  );
  ...
  const updateMutation = useMutation({
    mutationFn: (vars: { id: string; payload: Partial<Case> }) =>
      updateCase(vars.id, vars.payload, auth.slug || "default-tenant"),
  ...
  const filteredCases = useMemo(() => {
    return cases.filter((c) => {
      ...
      const repMatch = repFilter === "All" || c.assignedRep === repFilter;

      return (
        searchMatch && statusMatch && priorityMatch && typeMatch && repMatch
      );
    });
  }, [cases, search, statusFilter, priorityFilter, typeFilter, repFilter]);

  const getSelectedCases = () =>
    Object.keys(rowSelection)
      .map(Number)
      .map((index) => filteredCases[index]);
    ...
    try {
      await Promise.all(
        selected.map((c) =>
          updateMutation.mutateAsync({
            id: c.id,
            payload: { assignedRep: newAssignee },
          }),
        ),
      );
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      setSuccessModalOpen(true);
      if (selectedCase?.id === caseToAssign.id)
        setSelectedCase({ ...selectedCase, assignedRep: newAssignee });
    } catch (err) {
  ...
  const handleImportSubmit = async (validRows: any[]) => {
    await Promise.all(
      validRows.map((row) =>
        createCase(auth.slug || "default-tenant", row as Partial<Case>),
      ),
    );
  ...
  const stats = useMemo(
    () => ({
      total: cases.length,
      open: cases.filter((c) => c.status === "Open" || c.status === "Assigned")
        .length,
      inProgress: cases.filter(
        (c) =>
          c.status === "In Progress" || c.status === "Waiting For Customer",
      ).length,
    ...
    () => [
      ...
      columnHelper.accessor("priority", {
        ...
        cell: (info) => {
          ...
          return (
            <span
              className={`badge badge-sm font-semibold border-none ${colors[val]}`}
            >
              {val}
            </span>
          );
      ...
      columnHelper.accessor("status", {
        ...
        cell: (info) => {
          ...
          const colors: Record<string, string> = {
            ...
            "Waiting For Customer": "badge-warning",
            Resolved: "badge-success text-white",
            Closed: "badge-neutral",
            Escalated: "badge-error text-white",
          };
          return (
            <span
      ...
      columnHelper.display({
        ...
        cell: ({ row }) => (
          ...
          >
            ...
            >
              <li>
                <a onClick={() => setSelectedCase(row.original)}>
                  <MdSupportAgent size={16} /> View Details
                </a>
              </li>
              <li>
                <a
              ...
              <li>
                <a
                  ...
                  onClick={() =>
                    handleStatusChange(row.original.id, "Escalated")
                  }
                >
                  <MdTrendingUp size={16} /> Escalate
                </a>
              </li>
              <li>
                <a
                  className="text-error"
                  onClick={(e) => initiateDelete(e, row.original.id)}
                >
  ...
  return (
    <div className="min-h-screen bg-base-200 p-4 md:p-6 font-sans flex flex-col h-full overflow-hidden relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4 bg-base-100 p-3 rounded-xl border border-base-300 shadow-sm">
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative w-full md:w-80">
            ...
            <input
              type="text"
              placeholder="Search cases, customers, products..."
              value={search}
        ...
        <div className="flex bg-base-200 p-1 rounded-lg border border-base-300">
          ...
          <button
            ...
            className={`btn btn-sm btn-ghost px-3 ${view === "kanban" ? "bg-base-100 shadow-sm" : ""}`}
          >
            <MdViewKanban size={18} /> Kanban
          </button>
          <button
            onClick={() => setView("analytics")}
            className={`btn btn-sm btn-ghost px-3 ${view === "analytics" ? "bg-base-100 shadow-sm" : ""}`}
          >
      ...
      {Object.keys(rowSelection).length > 0 && view === "table" && (
        <div className="bg-primary/10 border border-primary/20 rounded-xl p-3 mb-4 flex items-center justify-between shadow-sm animate-fade-in-up">
          <span className="text-sm font-semibold text-primary">
            {Object.keys(rowSelection).length} cases selected
          </span>
                      ...
                      .map((c) => (
                        ...
                        >
                          <div className="flex justify-between text-xs mb-1">
                            ...
                            <span
                              className={`badge badge-xs ${c.priority === "Critical" ? "badge-error" : c.priority === "High" ? "badge-warning" : "badge-ghost"}`}
                            >
                              {c.priority}
                            </span>
                          </div>
                          ...
                          <span>{c.assignedRep?.split(" ")[0] || "Unassigned"}</span>
                            <span
                              className={
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-base-200/30 space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-base-100 border border-base-300 rounded-xl p-5 shadow-sm">
                  <h3 className="font-bold text-base-content mb-4 text-sm uppercase tracking-wider">
                    Cases by Status
                  </h3>
                  <div className="h-64">
                ...
                <div className="bg-base-100 border border-base-300 rounded-xl p-5 shadow-sm">
                  <h3 className="font-bold text-base-content mb-4 text-sm uppercase tracking-wider">
                    Representative Performance (Resolved Cases)
                  </h3>
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
      ...
      {/* ── Case Details Drawer ── */}
      <div
        className={`fixed inset-0 bg-black/40 z-[100] transition-opacity ${selectedCase ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
        onClick={() => setSelectedCase(null)}
      >
        <div
          className={`absolute right-0 top-0 h-full w-full md:w-[750px] bg-base-100 shadow-2xl transition-transform duration-300 transform ${selectedCase ? "translate-x-0" : "translate-x-full"} flex flex-col`}
          onClick={(e) => e.stopPropagation()}
        >
          {/* Drawer Header */}
          <div className="p-6 border-b border-base-300 bg-base-200/50 flex flex-col gap-4">
            <div className="flex justify-between items-start">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span
                    className={`badge badge-sm ${selectedCase?.status === "Resolved" ? "badge-success text-white" : selectedCase?.status === "Escalated" ? "badge-error text-white" : "badge-primary"}`}
                  >
                    {selectedCase?.status}
                  </span>
                </div>
                <h2 className="text-xl font-bold text-base-content leading-tight mt-2">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                <MdSupportAgent /> Reply to Customer
              </button>
              <button
              ...
              <button
                className="btn btn-outline btn-sm bg-base-100"
                onClick={() => {
                  setCaseToAssign(selectedCase);
                  setAssignModalOpen(true);
                }}
              >
                <MdAssignmentInd /> Reassign
              </button>
              <button
                className="btn btn-outline btn-sm btn-error bg-base-100"
                onClick={() =>
                  selectedCase &&
                  handleStatusChange(selectedCase.id, "Escalated")
                }
              >
                <MdTrendingUp /> Escalate
              </button>
                ...
                >
                  <li>
                    <a
                      ...
                      onClick={() =>
                        selectedCase &&
                        handleStatusChange(selectedCase.id, "Resolved")
                      }
                    >
                  ...
                  <li>
                    <a
                      onClick={() =>
                        selectedCase &&
                        handleStatusChange(selectedCase.id, "Closed")
                      }
                    >
                      <MdClose /> Close
                    </a>
                  ...
                  <li>
                    <a
                      className="text-error"
                      onClick={(e) =>
                        selectedCase && initiateDelete(e, selectedCase.id)
                      }
                    >
                      <MdDelete /> Delete
                    </a>
            ...
            <div className="tabs tabs-bordered w-full border-b border-base-300 mt-2">
              ...
              <a
                className={`tab font-medium flex items-center gap-1 ${activeTab === "comments" ? "tab-active text-primary" : ""}`}
              ...
              <a
                className={`tab font-medium ${activeTab === "attachments" ? "tab-active text-primary" : ""}`}
                onClick={() => setActiveTab("attachments")}
              >
                Attachments
              </a>
              <a
                className={`tab font-medium ${activeTab === "resolution" ? "tab-active text-primary" : ""}`}
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-base-100">
              ...
              <div className="space-y-8 animate-fade-in">
                {selectedCase?.slaStatus === "Breached" && (
                  <div className="alert alert-error shadow-sm py-2 rounded-xl text-white">
                    ...
                    <span className="text-sm font-bold">
                      SLA Breached! Resolution time exceeded the target.
                    </span>
                  </div>
                )}
                ...
                <section>
                  ...
                  <div className="grid grid-cols-2 gap-y-5 gap-x-6 text-sm">
                    <div className="col-span-2 bg-base-200/50 p-4 rounded-lg border border-base-200">
                      <p className="text-base-content/50 mb-1 font-semibold uppercase text-xs">
                        Description
                      </p>
                    ...
                    <div>
                      <p className="text-base-content/50 mb-1 text-xs uppercase font-semibold">
                        Priority
                      </p>
                    ...
                    <div>
                      ...
                      <p className="font-medium">{selectedCase?.assignedRep}</p>
                    </div>
                    <div>
                      <p className="text-base-content/50 mb-1 text-xs uppercase font-semibold">
                        Product
                ...
                <section className="grid grid-cols-2 gap-4">
                  <div className="bg-base-200/30 p-4 rounded-xl border border-base-200 text-xs space-y-2">
                    <p className="text-base-content/50 font-bold uppercase mb-2 border-b border-base-200 pb-1">
                      Dates & Deadlines
                    </p>
                  ...
                  <div className="bg-base-200/30 p-4 rounded-xl border border-base-200 text-xs space-y-2">
                    ...
                    <p className="flex justify-between">
                      <span className="text-base-content/60">SLA Status:</span>{" "}
                      <span
                        className={`font-bold ${selectedCase?.slaStatus === "On Track" ? "text-success" : "text-error"}`}
                      >
            ...
            {activeTab === "comments" && (
              <div className="animate-fade-in flex flex-col h-full gap-4">
                <div className="flex-1 space-y-4 overflow-y-auto mb-4 bg-base-200/20 p-4 rounded-xl border border-base-200">
                  ...
                  selectedCase.comments.length === 0 ? (
                    ...
                    selectedCase.comments.map((c) => (
                      <div
                        key={c.id}
                        className={`chat ${c.type === "Internal" ? "chat-end" : "chat-start"}`}
                      >
                        ...
                        <div
                          className={`chat-bubble text-sm ${c.type === "Internal" ? "chat-bubble-primary" : "bg-base-300 text-base-content"}`}
                        >
                          {c.type === "Internal" && (
                ...
                <div className="bg-base-200 p-4 rounded-xl border border-base-300">
                    ...
                    <div className="flex gap-2">
                      ...
                      <label className="cursor-pointer label p-0 gap-2">
                        <input
                          ...
                          name="ctype"
                          className="radio radio-xs radio-primary"
                          checked={commentType === "Customer"}
                          onChange={() => setCommentType("Customer")}
                        />
                        <span className="text-xs font-semibold">
                          Customer Reply
                        </span>
                  ...
                  <textarea
                    className={`textarea textarea-bordered w-full text-sm ${commentType === "Internal" ? "bg-primary/5 border-primary/30" : ""}`}
                    rows={3}
                    placeholder={
                      commentType === "Internal"
                        ? "Write an internal note..."
                        : "Reply to customer (Type @ to mention)..."
                    }
                    value={newComment}
            ...
            {activeTab === "attachments" && (
              <div className="animate-fade-in text-center py-12">
                ...
                />
                <h3 className="font-bold text-base-content/70">
                  No attachments
                </h3>
                <p className="text-sm text-base-content/50 mb-4">
                  Upload files related to this case.
                </p>
                ...
                selectedCase?.status === "Closed" ? (
                  <div className="bg-success/10 border border-success/20 rounded-xl p-5">
                    <h3 className="font-bold text-success flex items-center gap-2 mb-4">
                      <MdHistory /> Resolution Details
                    </h3>
                    <div className="space-y-4 text-sm">
                      <p>
                        <span className="font-semibold block text-xs uppercase text-success/70 mb-1">
                          Resolution Notes
                        </span>
                  ...
                  <div className="bg-base-200/50 border border-base-200 rounded-xl p-5 text-center py-10">
                    ...
                    <p className="text-sm text-base-content/50 mb-6">
                      Resolution details will appear here once the case is
                      resolved.
                    </p>
      ...
      <dialog className={`modal ${bulkStatusModalOpen ? "modal-open" : ""}`}>
        <div className="modal-box">
          <h3 className="font-bold text-lg mb-4">
            Update Status for Selected Cases
          </h3>
          <select
      ...
      <dialog className={`modal ${bulkAssignModalOpen ? "modal-open" : ""}`}>
        <div className="modal-box">
          ...
          <div className="dropdown w-full">
            ...
            >
              {newAssignee || "-Select Representative-"}
            </label>
            <div
              tabIndex={0}
              className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-full border border-base-300"
            >
              <input
                type="text"
                placeholder="Search..."
              ...
              <ul className="max-h-60 overflow-y-auto">
                {salesReps
                  .filter((r: string) =>
                    r.toLowerCase().includes(assigneeSearch.toLowerCase()),
                  )
                  .map((rep: string) => (
                    <li key={rep}>
                      <a
      ...
      <dialog className={`modal ${bulkDeleteModalOpen ? "modal-open" : ""}`}>
        <div className="modal-box">
          <h3 className="font-bold text-lg text-error flex items-center gap-2">
            <MdWarning /> Confirm Bulk Deletion
          </h3>
          <p className="py-4">
            Are you sure you want to delete {Object.keys(rowSelection).length}{" "}
            selected case(s)? This action cannot be undone.
          </p>
          <div className="modal-action mt-6">
            <button
              className="btn btn-ghost"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="form-control">
                  ...
                  <input
                    ...
                    value={caseToEdit.subject}
                    onChange={(e) =>
                      setCaseToEdit({ ...caseToEdit, subject: e.target.value })
                    }
                    required
                  />
                </div>
                <div className="form-control">
                  ...
                  <input
                    type="text"
                    className="input input-bordered w-full"
                    value={caseToEdit.customer}
                    onChange={(e) =>
                      setCaseToEdit({ ...caseToEdit, customer: e.target.value })
                    }
                    required
                  />
                </div>
                <div className="form-control">
                  ...
                  <input
                    type="text"
                    className="input input-bordered w-full"
                    value={caseToEdit.product || ""}
                    onChange={(e) =>
                      setCaseToEdit({ ...caseToEdit, product: e.target.value })
                    }
                  />
                </div>
                <div className="form-control">
                ...
                <div className="form-control">
                  ...
                  <select
                    className="select select-bordered w-full"
                    value={caseToEdit.type}
                    onChange={(e) =>
                      setCaseToEdit({ ...caseToEdit, type: e.target.value })
                    }
                  >
                    <option value="Complaint">Complaint</option>
                    <option value="Support Request">Support Request</option>
